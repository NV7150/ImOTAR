...
//     [SerializeField] private Unity.InferenceEngine.ModelAsset onnxModel;
...
//     private Unity.InferenceEngine.Tensor<float> _imgTensorGPU;            // [1,3,H,W]
//     private RenderTexture _fullOutputRT;            // [dstW x dstH] RFloat
//     private RenderTexture _resizeRefDepth, _refDepthCanvas; // RFloat
//     private Unity.InferenceEngine.Tensor<float> _refTensorGPU;                    // [1,1,H,W]
...
//             if (!hasMore) {
//                 var outRef = _worker.PeekOutput() as Unity.InferenceEngine.Tensor<float>;
//                 if (outRef == null)
...
//         var baseModel = Unity.InferenceEngine.ModelLoader.Load(onnxModel);
...
//     private Unity.InferenceEngine.Model BuildNormalizedModel(Unity.InferenceEngine.Model baseModel){
...
//         var fOuts = Unity.InferenceEngine.Functional.Forward(baseModel, fInputs);
...
//         var dMin = Unity.InferenceEngine.Functional.ReduceMin(depth, axes, true);
//         var dMax = Unity.InferenceEngine.Functional.ReduceMax(depth, axes, true);
//         var denom = dMax - dMin + normalizationEpsilon;
//         var norm  = Unity.InferenceEngine.Functional.Clamp((dMax - depth) / denom, 0f, 1f);

//         return g.Compile(norm);
//     }
// private Unity.InferenceEngine.Model BuildNormalizedAndCalibratedModel(Unity.InferenceEngine.Model baseModel){
//     var g = new Unity.InferenceEngine.FunctionalGraph();
//     var fInputs = g.AddInputs(baseModel); // image only
//     var refDepth = g.AddInput<float>(new Unity.InferenceEngine.DynamicTensorShape(1, DEPTH_CHANNELS, onnxHeight, onnxWidth));
...
//     // ---- フレーム内正規化（自然形）----
//     var axes = new int[] { 1, 2, 3 };
//     var rMin = Unity.InferenceEngine.Functional.ReduceMin(raw, axes, true);
//     var rMax = Unity.InferenceEngine.Functional.ReduceMax(raw, axes, true);
//     var denom = rMax - rMin + normalizationEpsilon;
//     var z0 = Unity.InferenceEngine.Functional.Clamp((raw - rMin) / denom, 0f, 1f); // [0,1]

//     // ---- 無効=-1 を除外する連続マスク（refDepth>tau のみ有効）----
//     var tauC = Unity.InferenceEngine.Functional.Constant(refDepthPositiveTau);
//     var diffValid = refDepth - tauC;
//     var posValid  = Unity.InferenceEngine.Functional.Clamp(diffValid, 0f, 1e9f);                 // ReLU
//     var maskValid = posValid / (posValid + normalizationEpsilon);          // 0..1

//     // ---- 向きの自動決定（z0 と refDepth の加重共分散の符号）----
//     var n0   = Unity.InferenceEngine.Functional.ReduceSum(maskValid, axes, false);
//     var Sx0  = Unity.InferenceEngine.Functional.ReduceSum(z0 * maskValid, axes, false);
//     var Sy0  = Unity.InferenceEngine.Functional.ReduceSum(refDepth * maskValid, axes, false);
//     var Sxx0 = Unity.InferenceEngine.Functional.ReduceSum(z0 * z0 * maskValid, axes, false);
//     var Sxy0 = Unity.InferenceEngine.Functional.ReduceSum(z0 * refDepth * maskValid, axes, false);

//     var invN0 = 1.0f / (n0 + normalizationEpsilon);
//     var mx0   = Sx0 * invN0;
//     var my0   = Sy0 * invN0;
//     var cov0  = Sxy0 * invN0 - mx0 * my0;
//     var sCov  = cov0 / (Unity.InferenceEngine.Functional.Abs(cov0) + normalizationEpsilon);      // ~sign(cov0)

//     var half = Unity.InferenceEngine.Functional.Constant(0.5f);
//     var one  = Unity.InferenceEngine.Functional.Constant(1f);
...
//     var zNearTh = Unity.InferenceEngine.Functional.Constant(0.05f);
//     var zFarTh  = Unity.InferenceEngine.Functional.Constant(0.95f);
//     var anchorScale = Unity.InferenceEngine.Functional.Constant(0.10f); // 0.1 * (#帯域画素) を疑似サンプル重みとする

//     // 近側マスク: zNearTh - z の ReLU を0..1へ
//     var mNeStep = Unity.InferenceEngine.Functional.Clamp(zNearTh - z, 0f, 1e9f);
//     var mNeBand = (mNeStep) / (mNeStep + normalizationEpsilon);
//     var mNe     = mNeBand * maskValid;

//     // 遠側マスク: z - zFarTh の ReLU を0..1へ
//     var mFaStep = Unity.InferenceEngine.Functional.Clamp(z - zFarTh, 0f, 1e9f);
//     var mFaBand = (mFaStep) / (mFaStep + normalizationEpsilon);
//     var mFa     = mFaBand * maskValid;

//     var nNe = Unity.InferenceEngine.Functional.ReduceSum(mNe, axes, false);                        // 近側サンプル数
//     var nFa = Unity.InferenceEngine.Functional.ReduceSum(mFa, axes, false);                        // 遠側サンプル数

//     var yNe = Unity.InferenceEngine.Functional.ReduceSum(refDepth * mNe, axes, false) / (nNe + normalizationEpsilon); // 近側平均
//     var yFa = Unity.InferenceEngine.Functional.ReduceSum(refDepth * mFa, axes, false) / (nFa + normalizationEpsilon); // 遠側平均

//     var wNe = anchorScale * nNe;     // 近側アンカー重み（疑似サンプル数）
//     var wFa = anchorScale * nFa;     // 遠側アンカー重み

//     // アンカーは (z=0, y=yNe), (z=1, y=yFa) の2点を追加したのと等価
//     var n    = Unity.InferenceEngine.Functional.ReduceSum(maskValid, axes, false);
//     var Sx   = Unity.InferenceEngine.Functional.ReduceSum(z * maskValid, axes, false);
//     var Sy   = Unity.InferenceEngine.Functional.ReduceSum(refDepth * maskValid, axes, false);
//     var Sxx  = Unity.InferenceEngine.Functional.ReduceSum(z * z * maskValid, axes, false);
//     var Sxy  = Unity.InferenceEngine.Functional.ReduceSum(z * refDepth * maskValid, axes, false);
...
//             var abd = Unity.InferenceEngine.Functional.Sqrt(r * r + normalizationEpsilon);
//             var w   = Unity.InferenceEngine.Functional.Clamp(huberDelta / abd, 0f, 1f); // Huber重み

//             var Sw   = Unity.InferenceEngine.Functional.ReduceSum(maskValid * w, axes, false);
//             var Swx  = Unity.InferenceEngine.Functional.ReduceSum(maskValid * w * z, axes, false);
//             var Swy  = Unity.InferenceEngine.Functional.ReduceSum(maskValid * w * refDepth, axes, false);
//             var Swxx = Unity.InferenceEngine.Functional.ReduceSum(maskValid * w * z * z, axes, false);
//             var Swxy = Unity.InferenceEngine.Functional.ReduceSum(maskValid * w * z * refDepth, axes, false);
...
//     var meters = Unity.InferenceEngine.Functional.Clamp(alpha * z + beta, 0f, 1e9f);
...
//     private void ResolveNamesFromModel(Unity.InferenceEngine.Model model){
//         // Input
...
//     private void ResolveInputIndicesFromRuntimeModel(Unity.InferenceEngine.Model model){
...
//                 var tfDEP = new TextureTransform().SetDimensions(_dstW, _dstH, 1).SetTensorLayout(Unity.InferenceEngine.TensorLayout.NCHW);
...
//         _imgTensorGPU = new Unity.InferenceEngine.Tensor<float>(new Unity.InferenceEngine.TensorShape(1, RGB_CHANNELS, _dstH, _dstW));
//         if (useCalibration) {
...
//             _refTensorGPU = new Unity.InferenceEngine.Tensor<float>(new Unity.InferenceEngine.TensorShape(1, DEPTH_CHANNELS, _dstH, _dstW));
//         }
